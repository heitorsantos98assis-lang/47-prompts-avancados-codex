# Prompt 01 — Criar AGENTS.md Perfeito para o Projeto

**Categoria:** Setup e Fundamentos
**Objetivo:** Gerar um `AGENTS.md` realmente util na raiz do projeto, que sirva como memoria persistente e reduza drasticamente a necessidade de repetir instrucoes em toda sessao.
**Quando usar:** No primeiro dia em que voce comeca a usar Codex em um projeto, ou quando seu `AGENTS.md` atual virou uma bagunca cheia de instrucoes obsoletas.

---

## Por que este prompt existe

O `AGENTS.md` e o arquivo mais importante do setup do Codex. Ele e carregado automaticamente em toda sessao e vira parte do contexto. Um `AGENTS.md` bem feito economiza tokens (voce nao repete instrucao), aumenta assertividade (Codex nao inventa convencoes) e evita erros bobos (ele ja sabe onde rodar testes, como fazer commit, o que nao mexer).

Um `AGENTS.md` mal feito e longo demais, cheio de obvio, com instrucoes que o Codex ja tira do proprio codigo. Ou pior: tem informacao desatualizada que confunde.

Este prompt faz o Codex auditar seu projeto, levantar as coisas que NAO sao obvias da leitura do codigo, e escrever um `AGENTS.md` enxuto e cirurgico.
...
```
Quero que voce crie (ou refaca) o arquivo AGENTS.md deste projeto.
...
4. Verifique se ja existe um AGENTS.md. Se existir, leia inteiro antes de sugerir substituicao.

Depois de investigar, escreva um AGENTS.md com estas secoes, nesta ordem, SEM inventar nada que voce nao tenha observado:
...
- Apenas pastas que o Codex precisa conhecer para navegar. Com 1 linha de descricao por pasta.
...
### Comandos que o Codex NAO DEVE Rodar Sem Pedir
...
- Caminhos de arquivos-chave que servem de referencia para o Codex.
...
**Monorepo:** adicione ao final do prompt: *"Este e um monorepo. Crie um AGENTS.md na raiz com visao geral + lista de pacotes, e sugira AGENTS.md por pacote quando fizer sentido."*
...
**Equipe com convencoes rigidas:** cole as convencoes existentes antes do prompt e peca para incluir na secao apropriada em vez de o Codex inferir.
...
- Apos o Codex gerar, leia linha por linha. Remova qualquer coisa que voce nao concorde antes de commitar.
- Commit o `AGENTS.md` no repo. Ele deve ser versionado.
- Reaplique este prompt sempre que a stack mudar (nova ferramenta, novo framework, nova ferramenta de teste).

## Sinal de que deu certo

Em uma proxima sessao, peca qualquer tarefa trivial ao Codex (ex: "rode os testes") e observe se ele ja sabe o comando exato sem perguntar. Se sabe, seu `AGENTS.md` esta fazendo o trabalho.
