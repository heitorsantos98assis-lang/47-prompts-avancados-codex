...
**Objetivo:** Identificar tarefas repetitivas que voce executa manualmente toda vez que o Codex termina uma mudanca (rodar lint, rodar testes, checar typecheck, formatar) e transformar isso em `hooks` configurados em `settings.json`, de forma que o proprio harness execute automaticamente.
...
```
Quero configurar hooks no `.codex/settings.json` deste projeto para automatizar tarefas repetitivas que hoje eu peco manualmente toda sessao.
...
2. Verifique se `.codex/settings.json` ja tem secao `hooks`. Se tiver, nao sobrescreva: merge.
...
- Mostre o JSON que entraria em `.codex/settings.json`.
...
- Versione `.codex/settings.json` no git para todos os devs do time herdarem.
